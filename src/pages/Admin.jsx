...
function ProductsView({ token: tokenProp }) {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const freshToken = useAuthStore.getState().token;
      const response = await fetch(`/api/products/all?search=${search}`, {
        headers: { 'Authorization': `Bearer ${freshToken}` }
      });
      ...
      {formOpen ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col">
                ...
                <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="bg-brand-cream border px-3 py-2 rounded-md focus:outline-none cursor-pointer" required>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2 max-w-md select-none font-sans">
            ...
            />
            <button onClick={fetchProducts} className="bg-brand-dark text-brand-cream px-4 py-2 rounded-md font-semibold text-xs">Search</button>
          </div>
                    ...
                    <tr key={prod._id} className="hover:bg-brand-cream/10">
                      ...
                      <td className="px-4 py-2 text-center select-none text-2xs font-semibold">
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-2">
                          <select
                            value={prod.isActive && prod.stock > 0 ? 'available' : 'out_of_stock'}
...
function ReviewsView({ token }) {
  ...
  return (
    <div className="space-y-6 text-left">
      <h2 className="font-display font-bold text-brand-dark text-xl border-b pb-3 select-none">Reviews Moderation</h2>
          ...
          <tbody className="divide-y divide-brand-border/40 font-sans text-xs">
            {reviews.map(rev => (
              <tr key={rev._id} className="hover:bg-brand-cream/10">
                <td className="px-4 py-3 text-left">
                  <span className="block font-bold text-brand-dark">{rev.product?.name}</span>
...
function SettingsView({ token }) {
  ...
  return (
    <div className="space-y-6 text-left max-w-2xl font-sans text-xs">
